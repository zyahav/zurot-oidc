...
| T-012 | Production deploy gate — GitHub protected environment + Cloudflare Pages | PLANNED | docs/implementation/t-012-spec.md | CI passes, deploy waits for human approval, no bypass path exists | Team | codex/prod-deploy-gate | 3 manual steps in GitHub + Cloudflare dashboard required |
| T-011 | Account PIN gate for Manage Profiles (parental boundary, Netflix model) | DONE | docs/implementation/t-011-spec.md | PIN setup, unlock, cooldown, recovery via email OTP, 14/14 QA | Team | merged to main | Merge commit 8b0f781. PIN setup, entry, 30min unlock, Resend recovery OTP. QA 14/14. |
| T-010 | Root route 404, sign-out modal persists, manage gate uses env var not Clerk | DONE | docs/implementation/t-010-spec.md | / redirects correctly, sign-out modal clears, manage gate is honest placeholder, lint 0 errors, qa-run2 14/14 | Team | merged to main | Merge commit 849e785. Gate placeholder pending T-011. |
