...
|---|---|---|---|---|---|---|---|
| T-001 | Set up CI/CD pipeline (lint + build on push/PR) | PLANNED | BASELINE.md, WORKFLOW.md | `npm run lint`, `npm run build` in CI | Team | `codex/ci-cd-bootstrap` | First task after baseline stabilization |
| T-002 | Add minimal smoke tests for OIDC contract | PLANNED | BASELINE.md | Discovery, JWKS, token invalid request, userinfo unauthorized | Team | `codex/smoke-tests` | Add scripts suitable for CI |
