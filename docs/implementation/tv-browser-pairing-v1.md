...
1. A television opens `/tv` and receives a QR code plus an eight-character
   ...
   `/devices` and types the visible code in **Connect a TV**. The short
   `/tv/connect` address redirects to that canonical device-management screen.
...
6. An owner connects and revokes televisions from `/devices`. The television also has a local sign-out action.
...
- Manual code resolution requires a signed-in account owner. The resolver
  ...
  still-pending, unexpired pairing. Approval happens within the already
  owner-PIN-unlocked Manage Devices session, so the owner is not prompted twice.
